...
export default function CinematicThemeToggle({ className = "" }) {
  ...
  const timerInRef = useRef(null);
  const timerOutRef = useRef(null);

  const cleanup = useCallback(() => {
    if (timerInRef.current) clearTimeout(timerInRef.current);
  ...
  const handleClick = () => {
    ...
    setGoingDark(targetIsDark);
    setPhase("in");

    timerInRef.current = setTimeout(() => {
      toggleTheme();
  ...
  return (
    <>
      <button
        ...
        className={`relative inline-flex items-center justify-center p-2 rounded-xl
        ...
        <AnimatePresence mode="wait">
          {isDark ? (
            ...
            <motion.div
              ...
              transition={{ duration: DUR.micro, ease: EASE.out }}
            >
              <Moon className="w-5 h-5 text-secondary-600 dark:text-secondary-400" aria-hidden="true" />
            </motion.div>
          )}
        </AnimatePresence>
      </button>
