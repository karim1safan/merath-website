...
import useNamesQuiz from '../hooks/useNamesQuiz';
...
const NamesQuizPage = () => {
  ...
  const { questions, loading, error } = useNamesQuiz(10);
  ...
  if (error) {
    return (
      <EmptyState
        icon={<Star className="w-16 h-16 text-red-500" />}
  ...
  if (questions.length === 0) {
    return (
      <EmptyState
        ...
        title="لا توجد أسئلة"
        description="لم يتم العثور على أسئلة أسماء الله الحسنى"
        actionLabel="العودة"
        onAction={() => navigate(ROUTES.HOME)}
      />
    );
    ...
    return (
      <div className="max-w-md mx-auto text-center py-12">
        ...
        <p className="text-secondary-600 dark:text-secondary-400 mb-2">
          {questions.length} سؤال عن أسماء الله الحسنى التسعة والتسعين
        </p>
        <p className="text-sm text-secondary-500 dark:text-secondary-400 mb-8">
          تعرف على المعاني والكتابات اللاتينية
        </p>
